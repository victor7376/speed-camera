...
def make_graph_image(total_by, days_prev, speed_over):
    ...
    graph_title = ('Previous %s days AVE %s by %s for SPEEDS >= %s %s\n%s' %
                   (str(days_prev), speed_units.upper(), total_by, str(speed_over), speed_units, now))
    if DEBUG:
    ...
    plt.ylabel('AVE ' + speed_units.upper() + ' by ' + total_by)
    plt.plot(xd, y)
